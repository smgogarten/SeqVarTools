...
\Rpackage{SeqArray} provides an alternative to the Variant Call
...
\Rpackage{SeqVarTools} provides an interface to the \Rpackage{SeqArray} storage
format with tools for many common tasks in variant analysis.

It is highly recommended to read the vignette in \Rpackage{SeqArray}
...
To work with \Rpackage{SeqVarTools}, we must first convert a VCF file
into the \Rpackage{SeqArray} GDS format.  All information in the VCF
...
The variant IDs are sequential integers created by \Rcode{seqVCF2GDS}.
...
\Rcode{getGenotype} transforms the genotypes from the internal storage
...
\Rcode{getGenotypeAlleles} returns the nucleotides instead of integers.
<<>>=
...
one time.
We can use \Rcode{applyMethod} to select a subset of variants and/or
samples.  \Rcode{applyMethod} is essentially a wrapper for
\Rcode{seqSetFilter} that enables us to apply a method or function to
...
instead of using \Rcode{applyMethod}.
<<>>=
...
object to select a range on chromosome 22.  This feature is not available in \Rcode{seqSetFilter}.
<<>>=
...
filter for both methods, we set it once using \Rcode{seqSetFilter} and
reset at the end.
<<fig=TRUE>>=
...
which can reveal variants of low quality.
<<fig=TRUE>>=
pval <- hwe(gds)
